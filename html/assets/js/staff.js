...
// Sample staff data
const staffData = [
  { id: 1, fullName: 'Nguyễn Văn A', role: 'Lễ tân', phone: '0901234567', email: 'nva@resort.com' },
  { id: 2, fullName: 'Trần Thị B', role: 'Phục vụ phòng', phone: '0901234568', email: 'ttb@resort.com' },
  { id: 3, fullName: 'Phạm Văn C', role: 'Bảo vệ', phone: '0901234569', email: 'pvc@resort.com' },
  { id: 4, fullName: 'Lê Thị D', role: 'Kế toán', phone: '0901234570', email: 'ltd@resort.com' }
];
...
function renderStaffTable() {
  ...
  staffData.forEach(staff => {
    ...
    row.innerHTML = `
      ...
      <td>${staff.email}</td>
      <td>
        <button onclick="editStaff(${staff.id})" class="btn btn-primary">Sửa</button>
        <button onclick="deleteStaff(${staff.id})" class="btn btn-secondary">Xóa</button>
      </td>
    `;
...
menuItems.forEach(item => {
  item.addEventListener('click', function() {
    ...
    if (target === 'staff') {
      renderStaffTable();
      document.getElementById('roomTypesGrid').style.display = 'none';
      document.getElementById('roomsGrid').style.display = 'none';
      document.getElementById('panelStaff').style.display = 'block';
    } else {
      document.getElementById('panelStaff').style.display = 'none';
      document.getElementById('roomTypesGrid').style.display = 'grid';
    }
  });
});
...
staffForm.addEventListener('submit', function(e) {
  ...
  if (formData.id) {
    ...
    if (index !== -1) {
      staffData[index] = { ...staffData[index], ...formData };
    }
  } else {
    // Add new staff
    formData.id = staffData.length > 0 ? Math.max(...staffData.map(s => s.id)) + 1 : 1;
